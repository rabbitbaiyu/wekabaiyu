 ...
 */
public class IB1CN extends Classifier implements UpdateableClassifier, TechnicalInformationHandler {
	...
	public double classifyInstance(Instance instance) throws Exception {
		...
		int closest = (int)(similarity*0.3);
		System.out.println("number of instances		" + newm_Train.numInstances());
	...
	private int distance(Instance first, Instance second) {
		...
		for (int i = 0; i < m_Train.numAttributes(); i++) {
			...
			if (m_Train.attribute(i).isNominal()) {
				 ...
				 * ((int)first.value(i) != (int)second.value(i))) { distance +=
				 * 1; }
				 */
			 ...
			 * else {
			 * 
			 * // If attribute is numeric if (first.isMissing(i) ||
			 * second.isMissing(i)){ if (first.isMissing(i) &&
			 * second.isMissing(i)) { diff = 1; } else { if
			 * (second.isMissing(i)) { diff = norm(first.value(i), i); } else {
			 * diff = norm(second.value(i), i); } if (diff < 0.5) { diff = 1.0 -
			 * diff; } } } else { diff = norm(first.value(i), i) -
			 * norm(second.value(i), i); } distance += diff * diff; }
			 */
	...
	public static void main(String[] argv) throws Exception {
		runClassifier(new IB1CN(), argv);

		/*
		 * String filepath ="F:/系统备份/weka-src/data/weather.nominal1.arff";
		 * IB1copy ib2 = new IB1copy(); Instances ins =
		 * ib2.getinstance(filepath); Instance inc2 = ins.instance(2);
		 * //System.out.println(inc2); Instance inc3 = ins.instance(3);
		 * //System.out.println(inc3); // System.out.println(ins);
		 * System.out.println("----------");
		 * ins.setClassIndex(ins.numAttributes() - 1); System.out.println(ins);
		 * //ins.add(inc2); System.out.println("---------- instance begin");
		 * System.out.println(ins); System.out.println("---------- instance end"
		 * ); // double distance = ib2.distance(inc2, inc3); //
		 * System.out.println("distance=="+distance); ib2.buildClassifier(ins);
		 * // ib2.buildClassifier(ins); // ib2.classifyInstance(inc2);
		 * System.out.println("---------- classify instance3 ");
		 * System.out.println(inc3); //System.out.println(
		 * "---------- instance end"); double dis = ib2.classifyInstance(inc3);
		 * System.out.println("hello world	"+dis);
		 * 
		 * System.out.println();
		 * 
		 * //System.out.println("hello world	"); // System.out.println(dis);
		 * for (int k = 0; k < dis.length; k++) { System.out.println(
		 * "distributtion "+k+"	:"+dis[k]); }
		 */
